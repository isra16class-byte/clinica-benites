...
## [2026-09-02] Hosting: `Dockerfile`/`compose.prod.yaml` de producción para probar en Oracle Cloud

Se agregó `Dockerfile` (multi-stage: `npm run build` de assets, `composer install` sin dev, imagen final `php:8.3-fpm-alpine` con nginx+php-fpm en un solo contenedor vía `docker/supervisord.conf`), `compose.prod.yaml` (app + MySQL, sin Redis/Meilisearch/Mailpit/Selenium — no se usan en producción), `docker/entrypoint.sh` (espera a MySQL, genera `APP_KEY` si falta, corre migraciones y cachea config/rutas/vistas/eventos al arrancar), `docker/php/{www.conf,opcache.ini}`, `docker/nginx/default.conf` y `.dockerignore`. Guía completa de deploy en `docs/DEPLOY_ORACLE.md` (creación de la VM Ampere, firewall doble de Oracle, instalación de Docker, checklist de producción pendiente como HTTPS/backups). **Sin poder probarse en el sandbox de Claude**: no hay Docker instalado y la red está restringida a un listado corto de dominios que no incluye Docker Hub ni `fonts.bunny.net` (necesario para el build de Vite) — validado solo sintaxis (YAML, `sh -n` del entrypoint) y revisión manual contra la documentación de cada herramienta. Queda pendiente la primera prueba real en la VM de Oracle.
...
## [2026-09-02] Fix: `facturas.total` sin default rompía la creación de cualquier factura nueva

Error real reportado por el usuario al correr `db:seed --class=DemoHistoricoSeeder` en su entorno tras aplicar los parches de facturación SRI: `SQLSTATE[HY000]: 1364 Field 'total' doesn't have a default value`. Causa raíz: la migración del 01 sep que renombró `monto`→`total` le agregó `->default(0)` a `subtotal`/`iva` pero no a `total`, que se quedó `decimal(10,2)` NOT NULL sin default. El diseño nuevo crea la Factura antes de tener líneas (el total se calcula recién al guardar la primera línea, vía `LineaFactura::booted()`), así que el insert inicial siempre fallaba — no solo en el seeder: el formulario real de Filament (`FacturaForm.php`) tiene el mismo patrón (tab "Líneas" solo existe en Editar, no en Crear), así que el bug también afectaba crear una factura nueva desde el panel. Migración nueva `2026_09_02_000000_add_default_to_total_column_facturas_table.php` con SQL crudo (`ALTER TABLE ... MODIFY`, no `->change()`, porque el proyecto no tiene `doctrine/dbal`) que agrega `DEFAULT 0` a `total`. Sin tocar `Factura`/`LineaFactura`/el seeder — el cálculo real sigue viniendo de `recalcularTotales()`, esto solo permite que el insert inicial (total temporal 0) no truene.

## [2026-09-01] Facturación electrónica SRI: patch local de compatibilidad Laravel 13 para dazza-dev/laravel-sri-ec

Al correr `composer update` en el entorno real (Sail), Composer rechazó la instalación: `dazza-dev/laravel-sri-ec` v1.0.0 (única versión publicada) declara soporte solo hasta Laravel 12, no 13 (que es lo que usa este proyecto) — confirmado que ni la rama sin tagear en GitHub lo actualizó, y que no hay ningún issue reportando esto. Se vendorizó una copia local del código fuente real del paquete en `packages/dazza-dev/laravel-sri-ec/` (solo se tocó una línea de su `composer.json` para agregar soporte `^13.0`, ningún archivo PHP) y se registró como Composer "path repository" en el `composer.json` raíz. Revisado el `ServiceProvider` del paquete: usa únicamente APIs estables de Laravel, sin nada específico de versión — el bloqueo es con alta probabilidad solo metadata desactualizada, pero sigue sin poder probarse en este sandbox. Detalle completo y cuándo dejar de usar el patch (apenas el autor publique soporte oficial): `packages/dazza-dev/laravel-sri-ec/PATCH_LOCAL.md` y `docs/FACTURACION_ELECTRONICA_SRI.md`.

## [2026-09-01] Facturación electrónica SRI: Parte 2 (integración real con el paquete) escrita — sin poder probarse

Continuación de la entrada anterior. Se escribió la integración real con `dazza-dev/laravel-sri-ec`: `config/clinica.php` (datos tributarios de la empresa, todos placeholder), bloque nuevo en `.env.example` (RUC, razón social, establecimiento/punto de emisión, certificado .p12, ambiente pruebas/producción), `dazza-dev/laravel-sri-ec` agregado a `composer.json`, `App\Services\Sri\FacturaSriMapper` (arma el array `$documentData` exacto que pide el paquete, verificado contra su código fuente real) y `App\Services\Sri\FacturaSriService` (`motivosBloqueoEmision()` valida todo lo verificable sin red — paquete instalado, certificado/RUC configurados, factura con líneas y forma de pago —, `emitir()` asigna el secuencial recién al emitir y llama al cliente real del paquete), más una acción "Emitir al SRI" en `FacturasTable` (solo admin, avisa con motivos en español si no se puede emitir todavía). Investigación adicional: se clonó también `dazza-dev/sri-sender` para confirmar la forma exacta de la respuesta de autorización del SRI (`numeroAutorizacion`, `comprobante`, `fechaAutorizacion`). **Nada de esto se puede probar en este sandbox** (sin PHP/Composer) — sigue bloqueado del lado del cliente por el certificado .p12 y el RUC/establecimiento/punto de emisión pendientes. Ver `docs/FACTURACION_ELECTRONICA_SRI.md` para el detalle completo y los próximos pasos.

## [2026-09-01] Facturación electrónica SRI: Parte 1 (estructura interna) construida — Parte 2 (integración real) sin empezar

Cliente confirmó que la clínica sí necesita facturación electrónica al SRI, pero **no tiene certificado .p12 ni RUC/establecimiento/punto de emisión todavía** (ambos trámites pendientes del lado del cliente). Con ese bloqueo real, se construyó completa la Parte 1 (cambio estructural interno, sin depender de nada externo): migración de `facturas` (`monto`→`total`, agrega `subtotal`/`iva`, reemplaza `metodo_pago` texto libre por `forma_pago` con códigos del catálogo oficial SRI), tabla nueva `lineas_factura` (detalle exigido por el SRI, con cálculo automático de subtotal/IVA por línea y recálculo automático del total de la factura), `tipo_identificacion` en `pacientes`. Actualizados en cascada: `FacturaForm`/`FacturasTable`/`FacturaResource` (+ `LineasFacturaRelationManager` nuevo), `PacienteForm`/`PacientesTable`, el PDF de factura (detalle + aviso de "no es un comprobante SRI válido todavía" mientras no esté autorizada), el seeder de demo, y los 4 widgets del dashboard que sumaban la columna vieja `monto`. Se investigó a fondo el paquete real `dazza-dev/laravel-sri-ec` (y sus dependencias `sri-ec`/`sri-xml-generator`) clonándolo en el sandbox — catálogos oficiales del SRI (formas de pago, tipos de identificación, tarifas de IVA) verificados contra su código fuente real, no inventados. Dato adicional verificado por búsqueda web: los servicios de salud están gravados con tarifa 0% de IVA en Ecuador (LRTI art. 55-56), no 15% — es el default en `LineaFactura`, sin confirmar con un contador. **La Parte 2 (integración real con el paquete: mapper, servicio de envío, config de datos tributarios, composer.json) no se empezó** — ver `docs/FACTURACION_ELECTRONICA_SRI.md`, documento de continuación con todo el detalle para la próxima sesión. Sin instalar `vendor/` ni correr migraciones en este sandbox (mismo límite de siempre) — pendiente de probarse en el entorno real.

## [2026-09-01] Dato: confirmación en vivo del módulo de Signos vitales — expediente clínico completo cerrado

El usuario probó en su entorno real (tras aplicar el fix de namespace): pantalla "Ver" de Historia Clínica sin la sección "Signos vitales" cuando no hay datos cargados, sección visible y editable dentro de "Editar", guardado correcto (crea la fila en `signos_vitales`) y visible después en "Ver" con los datos cargados. Todo funcionando. Con esto, **los 3 módulos del expediente clínico completo (Alergias, Antecedentes, Signos vitales) quedan construidos y confirmados en el entorno real** — MEMORIA.md sección 8 actualizada. Sin cambios de código — solo confirmación.

## [2026-09-01] Fix: namespace incorrecto de `Section` rompía la pantalla "Ver" de Historia Clínica

Error real reportado por el usuario al entrar a Ver una historia clínica: `Class "Filament\Infolists\Components\Section" not found`. Causa raíz verificada clonando el código fuente real de `filamentphp/filament` tag `v5.7.6` desde GitHub (la red del sandbox permite `github.com`, aunque no Packagist): en Filament v5, `Section` (junto con `Grid`, `Fieldset`, `Group`, `Tabs`, `Wizard`, y otros componentes de layout) es un componente **compartido** entre formularios e infolists y vive únicamente en `Filament\Schemas\Components\Section` — no existe una clase separada en `Filament\Infolists\Components` ni en `Filament\Forms\Components`. El bug del infolist (`HistoriaClinicaInfolist.php`, sección de Alergias) es preexistente a esta sesión — nunca se había probado la pantalla "Ver" de Historia Clínica, ni siquiera durante la confirmación en vivo del módulo de Alergias (esa prueba cubrió Crear/Editar, no Ver). El mismo error de namespace se había repetido al agregar la sección de Signos vitales al formulario (`HistoriaClinicaForm.php`, `Filament\Forms\Components\Section`, tampoco existe). Se revisó todo el proyecto (`grep`) buscando el mismo patrón con otros componentes de layout (Grid, Fieldset, Group, Tabs, Wizard) — no se encontraron más casos. Sin cambios de comportamiento ni de datos, solo los 2 imports corregidos a `Filament\Schemas\Components\Section`.
...
## [2026-08-31] Docs: recuperar propuesta del expediente clínico completo + marco legal HL7/MSP

Se había perdido en la condensación del 28 ago: la entrevista del 25 ago con Ysrael Calle confirmó que "digitalizar la mayor parte del historial clínico" significa expediente completo, no solo texto libre — quedan 3 módulos con diseño pensado y sin construir (Alergias, Antecedentes, Signos vitales). También se recupera el contexto legal ecuatoriano (LOPDP + Acuerdos Ministeriales del MSP que obligan HL7 para historia clínica electrónica, públicas y privadas) y el matiz de inventario multi-área (farmacia/quirófano/admisión/facturación). Se agrega todo a la sección 8 de `MEMORIA.md`.

## [2026-08-31] Docs: recuperar 2 features confirmadas sin documentar + corregir color primario desactualizado

Auditoría del `MEMORIA.md` condensado contra el código real (a pedido del usuario, tras encontrar el gap del overlay del video): el rediseño completo del topbar (navy sólido, logo blanco, buscador con placeholder, menú de usuario + página de perfil) y las tarjetas de KPI presionables con popover + diamante de color, ambos confirmados funcionando en el entorno real en su momento, no aparecían en ningún lado del `MEMORIA.md` actual. Se agregan a la sección 7. De paso se corrige "color primario Teal" (desactualizado, quedó así del texto original) a navy `#0C447C`, confirmado contra `AdminPanelProvider.php`.

## [2026-08-28] Hero: fix real definitivo del desvanecido — overlay `::after`, mask-image no confiable sobre `<video>`

Entrada que había quedado fuera al condensar `CHANGELOG.md`/`MEMORIA.md` ese mismo día. El fix anterior (elipse `50% 50% at 50% 50%`) seguía sin verse en el entorno real con el video reproduciéndose — causa: `mask-image` no es confiable sobre `<video>` en todos los navegadores/GPUs (con decodificación acelerada por hardware, el navegador puede pintar los frames reales por una vía que ignora la máscara del contenedor; solo se respeta sobre una imagen estática, por eso nunca se detectó en el sandbox). Fix: se agrega `.cb-hero-slideshow::after`, un degradado radial pintado ENCIMA del video (overlay, no recorte), transparente en el centro y navy sólido en los bordes — convive con la máscara alfa que ya tenían `.cb-hero-video`/`.cb-hero-video-poster`, no la reemplaza. **Confirmado por el usuario en su entorno real con el video reproduciéndose (31 ago 2026)**, con captura mostrando el hero sin marco recto visible.

...
- **Video del Hero (28 ago 2026)**: confirmado por el usuario en su entorno real que se veía demasiado tenue — se subió la opacidad final (0.72→0.88, `public.css`). Primer intento de ensanchar la máscara (18%→32% / 78%→92%) causó bordes visibles (el punto `transparent` de 78% está validado para terminar el desvanecido antes de los bordes medios del rectángulo). Fix y sucesivas vueltas a pedido del usuario: `transparent` se deja fijo en 78%, se agranda solo el núcleo sólido (`black` 18%→40%→58%→66%). Margen de desvanecido ya angosto (12 puntos) — próximo pedido de "más visible" conviene resolverlo con el usuario viéndolo en su entorno real antes de seguir angostando. Pendiente confirmar visualmente (mismo caveat de siempre: acá no se puede reproducir el `.mp4` en vivo).

...
Última actualización: 26 de agosto de 2026 — primera de 3 direcciones propuestas para pulir el "toque" visual del panel (el usuario sintió que, tras el rediseño del encabezado, seguía faltando personalidad). Se propusieron 3 direcciones (tipografía+jerarquía numérica, refinar tarjetas/gráficos, detalles de marca en toda la interfaz) y el usuario eligió empezar por la de menor riesgo: **tipografía + jerarquía numérica**. Se agregó Space Grotesk (autoalojada vía `@fontsource/space-grotesk`, mismo criterio que ya usa el proyecto para Inter — no depende de Google Fonts en producción) solo para el título de página (`.fi-header-heading`), los títulos de sección/widget (`.fi-section-header-heading`) y la cifra de los 4 KPIs del Dashboard (`.fi-wi-stats-overview-stat-value`, subida de tamaño y con tracking ajustado); el resto del panel (tablas, formularios, botones) sigue en Inter para no arriesgar legibilidad de datos clínicos/financieros. Selectores verificados contra el código fuente real de Filament v5.7.6, mismo criterio de siempre. **Aún sin confirmar por el usuario en el entorno real** — pendiente que aplique el patch, corra `npm install` (nueva dependencia) y recompile.

...
Última actualización: 26 de agosto de 2026 — se guardó en el repo el prompt de dirección de diseño del panel (`DISEÑO.md`, archivo nuevo), a pedido del usuario, que lo había traído como un `.md` aparte con la técnica recomendada por el cookbook de Anthropic (`prompting_for_frontend_aesthetics.ipynb`) adaptada a un panel clínico interno: guiar tipografía/color/movimiento por separado en vez de pedir "hazlo bonito", nombrar explícitamente qué defaults se evitan, y — a diferencia del enfoque original del cookbook (pensado para productos de cara al público, donde "sorprender y deleitar" es el objetivo) — priorizar acá claridad/confianza/velocidad de lectura por encima de originalidad, coherente con los 2 colores de marca (navy `#0C447C`, verde azulado `#0F6E56`) y el criterio de verificar selectores contra el código fuente real de Filament ya usado en todo el proyecto (ver sección 8.1 y el historial de branding/topbar). Se referencia desde este archivo en vez de duplicar el contenido, para que quede un solo lugar de verdad. Uso previsto: pegar el bloque de `DISEÑO.md` al pedir un cambio visual **grande** (no ajustes puntuales, donde ya alcanza el criterio de siempre).

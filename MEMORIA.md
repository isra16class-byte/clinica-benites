...
- **Fix visual de la página individual**: se corrige el hueco superior excesivo de la sección de especialidad y el contraste del enlace "← Volver a especialidades" sobre fondo claro, sin tocar `app/Support/Especialidades.php` ni los datos. La solución usa un override scoped en `resources/css/public.css` y un ajuste puntual de layout en `resources/views/especialidad.blade.php`, sin cambiar el estilo base del footer.
- **Nav público con reveal por scroll hacia arriba**: el header fijo del sitio público se oculta al bajar y reaparece al subir, con una línea sutil de acento en la base. La regla clave es que el nav no debe tapar el primer bloque de contenido ni cubrir el botón de retorno en páginas de detalle; la lógica vive en `resources/views/partials/nav.blade.php` y la parte visual en `resources/css/public.css`.

...
- **Expediente clínico completo — alcance confirmado por el cliente** (entrevista 25 ago 2026, Ysrael Calle): al preguntarle directamente, confirmó que "digitalizar la mayor parte del historial clínico" significa un expediente completo (antecedentes, alergias, signos vitales, resultados de exámenes, todo conectado) — no solo el `diagnostico`/`tratamiento`/`notas` en texto libre que hoy tiene `HistoriaClinica`. Resultados de exámenes con archivo adjunto **ya está cubierto** por `OrdenEstudio` (no hace falta módulo nuevo ahí). Orden de construcción: alergias → antecedentes → signos vitales (seguridad del paciente primero; los 2 primeros son más simples al ser "por paciente" en vez de "por consulta").
  ...
  - **(3) Signos vitales — construido, pendiente de confirmar en el entorno real (01 sep 2026)**: a diferencia de Alergias y Antecedentes, va **por consulta** (1 a 1 con `HistoriaClinica`), no por paciente — así que no tiene Resource/nav propia: se integró directo en `HistoriaClinicaForm` (`Section` con `->relationship('signosVitales')`, Filament carga/guarda los campos contra el registro relacionado automáticamente) y en `HistoriaClinicaInfolist` (sección de solo lectura, oculta si no se cargó nada). Modelo `SignosVitales` (historia_clinica_id único, presion_arterial, temperatura, frecuencia_cardiaca, frecuencia_respiratoria, peso, talla, saturacion_oxigeno, notas) — todos los campos nullable, no todas las consultas miden los 7. Hereda los permisos de `HistoriaClinicaResource` (no tiene los suyos propios, al no ser un recurso independiente). Seeder de demo (`DemoHistoricoSeeder::crearSignosVitales()`) agrega signos vitales a 3 de cada 4 historias clínicas de demo (a propósito deja algunas sin datos). **Sin instalar `vendor/` ni correr migraciones en este sandbox** (mismo motivo que los 2 módulos anteriores: sin red a Packagist ni PHP disponible) — código revisado a mano contra los patrones ya usados, pendiente de probarse en el entorno real. **Ojo particular con este módulo**: el patrón `->relationship()` sobre un `Section`/`Group` para relaciones 1 a 1 (`HasOne`) no se pudo verificar contra el código fuente de Filament en este sandbox (sin `vendor/`) — es un patrón estándar y documentado de Filament, pero es el primer uso en este proyecto de una relación 1 a 1 embebida en un formulario (los módulos anteriores usan RelationManagers, que es un patrón ya probado en el proyecto). Verificar con cuidado al probar: que al crear/editar una Historia Clínica con algún signo vital cargado, se cree/actualice la fila en `signos_vitales`, y que quede vacía (sin fila) si no se carga nada.
...
4. Confirmar en entorno real el módulo de Signos vitales (sección de Historia Clínica, campos + guardado de la relación 1 a 1) — con atención particular al patrón `->relationship()` sobre el `Section`, primera vez que se usa así en el proyecto (ver nota en sección 8).
5. Confirmar con el cliente la matriz de permisos de Alergias y Antecedentes (sección 10, hoy es una decisión propia por analogía con Historias Clínicas). Con los 3 módulos del expediente clínico completo ya construidos, no queda ningún módulo nuevo pendiente de diseño — el resto son confirmaciones con el cliente.

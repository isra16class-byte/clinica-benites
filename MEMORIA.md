...
Última actualización: 26 de agosto de 2026 — el usuario reportó, tras aplicar el fix anterior (cambiar el `box-shadow` de blur puro a blur+`ring`), que la línea empezó a verse **también arriba** de la fila de 4 KPIs, de forma uniforme, además de seguir abajo. Esto reveló la causa real que el fix anterior no había identificado: un `ring` (box-shadow sin offset, solo spread) se aplica por igual en los 4 lados de cada tarjeta, no solo abajo — y como las 4 tarjetas de esta fila tienen la misma altura y están alineadas, ese borde se lee como una línea continua en cualquier lado donde se aplique. **Conclusión**: no era un problema de blur vs. ring en particular — es que cualquier sombra o borde simétrico sobre una fila de tarjetas iguales se va a leer como una "línea que las toca a todas", más notorio todavía sobre el fondo gris-azulado de `.fi-main` (ajuste de contraste del 25 ago) que sobre el blanco por defecto de Filament. La separación entre tarjetas y fondo ya está resuelta con ese color de fondo — la sombra/ring resultó decorativa y redundante. **Solución definitiva**: se saca el `box-shadow` del todo de `.fi-section`/`.fi-wi-stats-overview-stat` (sin reemplazo — la separación visual ya la da el fondo). Para "Por cobrar" (`cb-stat-destacado`), en vez de una sombra más marcada (que repetiría el mismo problema por ser también simétrica), se usa un acento de un solo lado: borde izquierdo de 3px en `var(--color-warning-600)` (variable real de Filament, confirmada en `packages/support/resources/css/index.css` de la v5.7.6, mismo naranja que ya usa esta tarjeta en su descripción) — al no ser simétrico, no puede alinearse con sus vecinas en una línea. **Aún sin confirmar por el usuario en el entorno real** — sin acceso a PHP/Sail/npm en esta sesión, verificado a mano solo el balance de llaves/paréntesis de `theme.css`. **Lección para el futuro**: cuando se corrige una sombra/borde para "eliminar una línea", conviene considerar de entrada que CUALQUIER efecto simétrico (blur o ring) sobre una fila de elementos iguales puede producir el mismo artefacto en cualquiera de sus lados — la solución robusta contra ese artefacto es evitar el efecto simétrico, no ajustar sus parámetros.

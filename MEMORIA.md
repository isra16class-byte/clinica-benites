...
- **Especialidades** (`sections/especialidades.blade.php`): directorio editorial de las 27 especialidades reales (`AreaSeeder.php`, única fuente), alfabetizado en 3 columnas — se descartó agrupar por categoría (quirúrgica/clínica/diagnóstico) por no venir confirmada esa taxonomía en el material del cliente. Rediseño: watermark "27" en trazo, marcador de grupo por letra, línea dorada trazada en hover. Segundo ajuste, mismo día (referencia visual del usuario: mobbin.com): el rail vertical fijo de letras A-U se reemplazó por `cb-directory-scatter` — esas mismas letras reales dispersas en los márgenes de la sección (posiciones por índice, no al azar; solo desde `xl`) — y se agregó `cb-stat-callout`, el "27" en primer plano con la escala tipográfica de la referencia. Pendiente confirmar visualmente en entorno real (mismo caveat de siempre, sin `vite build` completo en el sandbox).

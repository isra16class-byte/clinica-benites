...
Última actualización: 26 de agosto de 2026 — el usuario confirmó que la tipografía uniforme de "Por cobrar" (entrada de abajo) sí funcionó, pero aclaró que la captura que se había analizado en la sesión anterior ya tenía aplicado el fix de padding de la entrada de más abajo ("textos descuadrados y línea") — es decir, la línea gris bajo la fila de 4 KPIs **seguía viéndose** después de ese fix, no era la misma causa. Esto descarta el diagnóstico de esa entrada anterior sobre la línea (el `padding-block: 0.25rem` aplastado explicaba el descuadre de textos, pero no la línea) y confirma el diagnóstico original de la sesión de antes (pixel a pixel contra el fondo con Python/Pillow): el `box-shadow` de `.fi-section`/`.fi-wi-stats-overview-stat` seguía con 2 capas de blur puro y sin componente sólido tipo `ring` — nunca se había tocado en el fix de padding. Se corrigió ese `box-shadow` (y el de `cb-stat-destacado`, que tenía el mismo problema) volviendo al patrón real de Filament 5.7.6 (`shadow-sm ring-1`): 1 sombra suave con blur chico + 1 borde nítido de 1px sin blur. **Aún sin confirmar por el usuario en el entorno real** — sin acceso a PHP/Sail/npm en esta sesión, verificado a mano solo el balance de llaves/paréntesis de `theme.css`. **Lección para el futuro**: cuando el usuario comparte una captura después de aplicar un patch, conviene preguntar explícitamente "¿esta captura es de antes o de después del patch?" antes de dar por buena una causa raíz — en este caso se asumió sin preguntar que la imagen analizada era anterior al fix de padding, cuando en realidad ya lo tenía aplicado, lo que llevó a aceptar como resuelta una causa (padding) que no era la de la línea.

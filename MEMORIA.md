...
Última actualización: 23 de agosto de 2026 (branding del panel confirmado funcionando en el entorno real — logo, nombre y color primario turquesa OK. Rediseño del ajuste de color con criterio de 60/30/10 y sidebar oscuro con identidad propia, ver sección 8.1 — pendiente de probar esta versión).
...
- **Nota de riesgo**: las clases CSS usadas en ese `<style>` (`.fi-topbar`, `.fi-sidebar`, `.fi-sidebar-item.fi-active .fi-sidebar-item-button`, `.fi-sidebar-item-label`, `.fi-icon`, `.fi-sidebar-group-label`, `.fi-ta-header-cell`, `.fi-ta-table`, `.fi-ta-actions`) son las que usa Filament v3/v4 tradicionalmente. **Las primeras 4 ya se confirmaron funcionando** contra Filament v5.7 (el usuario mandó capturas donde se ve el efecto aplicado). Las nuevas de esta vuelta (`.fi-sidebar-item-label`, `.fi-icon`, `.fi-sidebar-group-label` dentro del sidebar oscuro) todavía no están confirmadas — si el texto/íconos del menú quedan invisibles (texto oscuro sobre fondo oscuro) es que esas clases puntuales cambiaron de nombre en v5, y hay que inspeccionar el HTML real del sidebar para ajustar el selector de texto/ícono.

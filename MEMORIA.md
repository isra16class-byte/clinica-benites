...
- [x] ~~**Sesión 3 — Alertas operativas**~~ — **construida** (25 ago 2026, décima entrada del día). El punto 4 de la propuesta (lotes de inventario vencidos/por vencer, facturas vencidas, camas ocupadas hace demasiado tiempo). Los 2 umbrales de negocio que faltaban se le preguntaron directo al usuario, que en ambos casos pidió que se decidiera un valor razonable en vez de darlo él — ver el detalle completo y los valores elegidos en la nueva sección **6.6.3** más abajo. **Aún sin confirmar por el usuario en el entorno real** — escrito sin acceso a Sail, se validó sintaxis con `php -l`, pero no se probó en `/admin`.

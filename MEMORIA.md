...
- **Hosting futuro — decisión en curso, no cerrada (02 sep 2026), primera prueba en marcha (mismo día)**: se comparó en pantalla con el usuario Hetzner vs. DigitalOcean (con crédito de GitHub Student Developer Pack, DigitalOcean cubre 11-33 meses gratis; la línea barata de Hetzner no está disponible para la cuenta del usuario y la que sí lo está sale más cara que DigitalOcean en specs equivalentes) — pero la opción que se está probando es **Oracle Cloud "Always Free"** (2 OCPU ARM + 12GB RAM, gratis para siempre, no un crédito que se agota), de sobra para el uso real estimado (500-700MB). Cuenta ya creada (tenancy `isra16`, región Bogotá). **`Dockerfile`/`compose.prod.yaml` de producción ya construidos** (un solo contenedor nginx+php-fpm vía supervisord + MySQL aparte, sin Redis/Meilisearch/Mailpit/Selenium — no se usan en producción; sesión/cola/caché ya son `database`). **Sin poder probarse en el sandbox de Claude** (sin Docker instalado, red restringida a un listado corto de dominios que no incluye Docker Hub ni fonts.bunny.net) — queda para confirmar en la VM real, igual que pasó antes con el bug de `facturas.total`. Guía completa de deploy (crear la VM, firewall doble de Oracle, instalar Docker, levantar el compose, crear el primer admin): `docs/DEPLOY_ORACLE.md`. Descartado por ahora: hardware físico en la clínica (riesgo de seguridad física con datos sensibles + sin backups automáticos). Dominio `.com` aún sin comprar. Detalle de la comparación de proveedores: `docs/SESION_2026-09-02_hosting_y_fix_facturacion.md`.
...
- **Facturación electrónica SRI — Parte 1 construida y confirmada funcionando en el entorno real (02 sep 2026), Parte 2 escrita sin poder probarse todavía**: el cliente confirmó que la clínica necesita emitir facturas electrónicas al SRI. Dos bloqueos de datos confirmados con el cliente: **no tiene certificado digital .p12 todavía** (hay que tramitarlo con una entidad certificadora — Security Data, BCE, Uanataca, etc., trámite con costo propio) y **no tiene RUC/establecimiento/punto de emisión asignados** (los asigna el SRI al registrar el punto de venta). Con esos dos bloqueos, se construyó completa la Parte 1 (estructura interna: líneas de factura, catálogos SRI de forma de pago/tipo de identificación/tarifa de IVA, PDF con detalle) **y también la Parte 2** (mapper + servicio que arman el documento y llaman a `dazza-dev/laravel-sri-ec`, acción "Emitir al SRI" en el panel). Al probar Parte 1 por primera vez en el entorno real (`composer update` + `migrate` + `db:seed`, 02 sep 2026) salió un bug real: `facturas.total` se quedó sin `default(0)` al renombrarse desde `monto` (a diferencia de `subtotal`/`iva`, que sí lo tienen), y el diseño crea la Factura antes de tener líneas — rompía tanto el seeder como crear una factura nueva real desde el panel. Corregido con una migración nueva (`2026_09_02_000000_add_default_to_total_column_facturas_table.php`, SQL crudo porque el proyecto no tiene `doctrine/dbal`). Con el fix aplicado, **el usuario confirmó Parte 1 funcionando de punta a punta**: seeder completo sin errores, listado `/admin/facturas` con totales reales por factura, y el botón "Emitir al SRI" bloqueando correctamente con el mensaje esperado (RUC/certificado faltantes + forma de pago solo en facturas `pagado`) — el bloqueo real para emitir de verdad sigue siendo únicamente los dos trámites pendientes del cliente, no código. **La Parte 2 (integración real con el paquete) sigue sin poder probarse** hasta que el cliente tenga certificado/RUC. Detalle completo, decisiones de diseño y pasos pendientes: `docs/FACTURACION_ELECTRONICA_SRI.md`.
...
- **`composer.lock` puede desincronizarse entre los patches de Claude y la máquina del usuario** — si el usuario corre `composer update` localmente (ej. para instalar una dependencia nueva) pero no commitea/pushea el `composer.lock` resultante, un patch posterior de Claude puede fallar al aplicar (`git am`) por desincronización, y una segunda máquina no podrá instalar sin repetir el `composer update` manual. Verificar siempre que `composer.lock` quede commiteado y pusheado después de cualquier cambio de dependencias hecho del lado del usuario.
...
5. Facturación electrónica SRI: Parte 1 ya probada y confirmada en el entorno real (02 sep 2026, ver sección 8). Del lado del cliente: tramitar el certificado .p12 y confirmar RUC/establecimiento/punto de emisión — recién con eso se puede probar la Parte 2 (`composer install` + publicar migraciones del paquete + probar "Emitir al SRI" en ambiente de pruebas del SRI). Detalle completo: `docs/FACTURACION_ELECTRONICA_SRI.md`.
6. Hosting (02 sep 2026, ver sección 2): `Dockerfile`/`compose.prod.yaml` ya listos, sin poder probarse en el sandbox — falta que el usuario cree la VM de Oracle Cloud (forma `VM.Standard.A1.Flex`), resuelva su firewall doble y corra el deploy siguiendo `docs/DEPLOY_ORACLE.md`; ahí es esperable que aparezca algún ajuste al probar por primera vez en el entorno real. Falta comprar el dominio `.com` y, después del primer deploy exitoso, agregar HTTPS y backups de MySQL (checklist en la sección 8 de `docs/DEPLOY_ORACLE.md`). Decisión de proveedor sigue sin cerrarse formalmente aunque Oracle Cloud es la opción que se está probando.

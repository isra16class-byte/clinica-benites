...
- **Hosting futuro — decisión en curso, no cerrada (02 sep 2026)**: se comparó en pantalla con el usuario Hetzner vs. DigitalOcean (con crédito de GitHub Student Developer Pack, DigitalOcean cubre 11-33 meses gratis; la línea barata de Hetzner no está disponible para la cuenta del usuario y la que sí lo está sale más cara que DigitalOcean en specs equivalentes) — pero la opción que se está evaluando en paralelo es **Oracle Cloud "Always Free"** (2 OCPU ARM + 12GB RAM, gratis para siempre, no un crédito que se agota), de sobra para el uso real estimado (500-700MB). Cuenta ya creada (tenancy `isra16`, región Bogotá). Próximo paso pendiente: crear la VM (forma `VM.Standard.A1.Flex`, no la que viene por default) con cuidado del firewall doble de Oracle. Descartado por ahora: hardware físico en la clínica (riesgo de seguridad física con datos sensibles + sin backups automáticos). Falta además un `Dockerfile`/compose de producción (hoy solo existe `compose.yaml` de Sail, para desarrollo) — independiente de qué proveedor se elija al final. Dominio `.com` aún sin comprar. Detalle completo: `docs/SESION_2026-09-02_hosting_y_fix_facturacion.md`.
...
6. Hosting (02 sep 2026, ver sección 2): crear la VM de Oracle Cloud (forma `VM.Standard.A1.Flex`) y resolver su firewall doble; armar el `Dockerfile`/compose de producción (no existe todavía); comprar el dominio `.com`. Decisión de proveedor sigue sin cerrarse formalmente aunque Oracle Cloud es la opción que se está probando.

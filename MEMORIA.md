...
- **Video del Hero (28 ago 2026)**: confirmado por el usuario en su entorno real que se veía demasiado tenue — se subió la opacidad final (0.72→0.88) y se ensanchó el área sólida de la máscara radial (18%→32% / 78%→92%) en `public.css`. Sigue pendiente confirmar visualmente este ajuste puntual (mismo caveat de siempre: acá no se puede reproducir el `.mp4` en vivo).
